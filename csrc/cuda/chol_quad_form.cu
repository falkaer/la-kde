...
__global__ void chol_quad_form_kernel(const scalar_t *W_triu_data, const scalar_t *Xn_data,
                                      const scalar_t *Xm_data, scalar_t *out_data,
                                      const int W_triu_stride1, const int W_triu_stride2, const int W_triu_stride3,
                                      const int Xn_stride1, const int Xn_stride2,
                                      const int Xm_stride1, const int Xm_stride2,
                                      const int out_stride1, const int out_stride2,
    ...
    __shared__ scalar_t Xm_block[16][16 + 1];      // [n, j] (+1 to avoid bank conflicts)
...
#pragma unroll
        ...
        for(int cj = 0; cj <= ci; cj += 16){ // triangular => cj <= ci
            if(nrow < N && cj + threadIdx.x < D)
                Xn_block[threadIdx.y][threadIdx.x] = __ldg(Xn_data + nrow * Xn_stride1 + (cj + threadIdx.x) * Xn_stride2);
            else
                Xn_block[threadIdx.y][threadIdx.x] = 0;
            
            if(mrow < M && cj + threadIdx.x < D)
                Xm_block[threadIdx.y][threadIdx.x] = __ldg(Xm_data + mrow * Xm_stride1 + (cj + threadIdx.x) * Xm_stride2);
            else
                Xm_block[threadIdx.y][threadIdx.x] = 0;
#pragma unroll
            for(int wj = 0; wj < 16; wj++)
                if(nrow < N && cj + wj < D && ci + threadIdx.x < D)
                    W_triu_blocks[wj][threadIdx.y][threadIdx.x] =
                            __ldg(W_triu_data + nrow * W_triu_stride1 + (cj + wj) * W_triu_stride2 + (ci + threadIdx.x) * W_triu_stride3);
                else
...
torch::Tensor
chol_quad_form_cuda(torch::Tensor W_triu, torch::Tensor Xn, torch::Tensor Xm, torch::Tensor out){
    CHECK_CUDA(W_triu);
    CHECK_CUDA(Xn);
    CHECK_CUDA(Xm);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND(torch::kHalf, W_triu.scalar_type(), "chol_quad_form_cuda", [&]{
        chol_quad_form_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
                W_triu.data_ptr<scalar_t>(),
...
static auto registry = torch::RegisterOperators()
        .op("lakde::chol_quad_form_cuda", &chol_quad_form_cuda);