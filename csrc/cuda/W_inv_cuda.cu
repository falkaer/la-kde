...
__global__ void W_inv_kernel(const rowptr_t *rowptr_data, const index_t *col_data,
                             const scalar_t *value_data, const scalar_t *Xn_data,
                             const scalar_t *Xm_data, scalar_t *out_data, const int N, const int D){
...
torch::Tensor W_inv_cuda(torch::Tensor rowptr, torch::Tensor col, torch::Tensor value,
                         torch::Tensor Xn, torch::Tensor Xm, torch::Tensor out){
    CHECK_CUDA(rowptr);
    CHECK_CUDA(col);
    CHECK_CUDA(value);
    CHECK_CUDA(Xn);
    ...
    auto blocks = dim3((32 * N + THREADS - 1) / THREADS, (D * (D + 1) / 2 + 31) / 32);
    auto stream = at::cuda::getCurrentCUDAStream();
    AT_DISPATCH_FLOATING_TYPES_AND(torch::kHalf, value.scalar_type(), "W_inv_cuda", [&]{
        if(rowptr.scalar_type() == torch::kInt32 && col.scalar_type() == torch::kInt32){
            W_inv_kernel<scalar_t, int32_t, int32_t><<<blocks, THREADS, 0, stream>>>(
                    rowptr.data_ptr<int32_t>(),
                    col.data_ptr<int32_t>(),
                    value.data_ptr<scalar_t>(),
                    Xn.data_ptr<scalar_t>(),
                    Xm.data_ptr<scalar_t>(),
                    out.data_ptr<scalar_t>(),
                    N, D);
        }else if(rowptr.scalar_type() == torch::kInt32 && col.scalar_type() == torch::kInt64){
            W_inv_kernel<scalar_t, int32_t, int64_t><<<blocks, THREADS, 0, stream>>>(
                    rowptr.data_ptr<int32_t>(),
                    ...
                    out.data_ptr<scalar_t>(),
                    N, D);
        }else if(rowptr.scalar_type() == torch::kInt64 && col.scalar_type() == torch::kInt32){
            W_inv_kernel<scalar_t, int64_t, int32_t><<<blocks, THREADS, 0, stream>>>(
                    ...
                    out.data_ptr<scalar_t>(),
                    N, D);
        }else if(rowptr.scalar_type() == torch::kInt64 && col.scalar_type() == torch::kInt64){
            W_inv_kernel<scalar_t, int64_t, int64_t><<<blocks, THREADS, 0, stream>>>(
                    rowptr.data_ptr<int64_t>(),
                    col.data_ptr<int64_t>(),
                    value.data_ptr<scalar_t>(),
                    Xn.data_ptr<scalar_t>(),
...
static auto registry = torch::RegisterOperators()
        .op("lakde::W_inv_cuda", &W_inv_cuda);
