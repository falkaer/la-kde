...
def plot_contour(xi, yi, zi, levels, norm, cmap, ax):
    colormesh = ax.pcolormesh(xi, yi, zi, shading='gouraud', cmap=cmap, norm=norm, antialiased=True)
    contours = ax.contour(xi, yi, zi, levels, colors='black', linestyles='solid',
                          norm=norm, zorder=2, antialiased=True)
...
def plot_posterior_contour(model, experiment, axes, f_levels=None, sd_levels=None, cmap='viridis'):
    _, f_plt, sd_plt, xi, yi, scales = _get_contour_predictions(model, 'nu_0', 'k', 'log_likelihood', None, 200)
...
if __name__ == '__main__':
    ...
    for k, v in hparams.items():
        ...
        print('Hyperparameter {} in range [{}, {}], optimal value: {}'.format(k, param.lower, param.upper, v))
    ...
    # f_levels = np.linspace(4, 8, 9)
    # sd_levels = np.linspace(0, 0.8, 9)
    
    # f_levels = np.linspace(130, 140, 9)
    # sd_levels = np.linspace(0, 2, 9)
