...
def torch_logpdf(model, Y, bsize):
    ...
    for i in tqdm(range(0, N, bsize)):
        for j in range(0, M, bsize):
            ...
            torch.logaddexp(result[j:j + bsize], torch.logsumexp(log_likelihood, dim=0), out=result[j:j + bsize])
...
if __name__ == '__main__':
    ...
    print('Running with dataset {} and bandwidth = {}'.format(args.dataset, args.bandwidth))
