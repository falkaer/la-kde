...
from lakde.utils import batched_knn_cov, compute_knn_graph, tensor_as, InterruptDelay

from lakde.linalg_util import chol_mm_trace, mvdigamma, mvlgamma, stabilized_cholesky, chol_logdet
...
def expect_log_q_lambda(nu, expect_logdet_lambda, logdet_W, D):
    ...
    return torch.sum(expect_log_B_q + (nu - D - 1) * expect_logdet_lambda / 2 - nu / 2 * D)
...
class LocalKNearestKDE(LocallyAdaptiveKDE):
    ...
    def init_parameters_(self, X, sparsity_threshold=None):
        ...
        if isinstance(self.k_or_knn_graph, int):
            ...
            self.knn_graph = compute_knn_graph(X, self.k_or_knn_graph, 1024, include_self=False, metric='l2',
                                               dtype=torch.int32)
        ...
        for i in range(num_blocks):
            self.rnm_blocks.append((inds[i], X.new_full((inds[i].size(1),), 1 / init_k)))
    ...
    def init_summaries_(self, X, sparsity_threshold=None):
        ...
        for bi, i in enumerate(range(0, N, self.block_size)):
            ...
            self.sigma_0_lambda_mm_trace[i:i + self.block_size] = nu * chol_mm_trace(sigma_0_tril, W_triu)
    ...
    def calc_prior_cov(self, X, block_idx):
        ...
        sigma_0 = batched_knn_cov(X, X[s:e], knn_graph.long(), include_self=False, self_center=True)
        sigma_0_tril, sigma_0_jitter[:] = stabilized_cholesky(sigma_0, sigma_0_jitter, verbose=self.verbose)
    ...
    def partial_step_(self, X, block_idx, sparsity_threshold=None):
        ...
        with InterruptDelay():
            ...
            self.rnm_blocks[block_idx] = (rnm_indices.to(self.knn_graph.dtype), rnm_values)
    ...
    def compute_elbo(self, X):
        ...
        expect_log_lambda_p = expect_log_p_lambda(self.nu_0, expect_tau, expect_log_tau,
                                                  expect_logdet_lambda, self.sigma_0_lambda_mm_trace,
                                                  self.logdet_sigma_0, D)
        ...
        elbo = expect_log_likelihood_no_rnm + expect_log_lambda_diff + expect_log_z_diff + expect_log_tau_diff
