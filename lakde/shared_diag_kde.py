...
def calc_log_rho_block(X_test, X_train, nu, w):
    D = X_test.size(1)
    expect_log_lambda = torch.sum(torch.digamma(nu) - torch.log(w))
    W_triu = torch.diag_embed(torch.sqrt(1 / w)).expand(X_train.size(0), D, D)
    exponent = -0.5 * nu
...
class SharedDiagonalizedKDE(AbstractKDE):
    ...
    def _partial_reestimation_step(self, X, bj, sparsity_threshold=None):
        ...
        for bi, i in enumerate(range(0, N, self.block_size)):
            ...
            torch.logaddexp(rnm_log_consts, torch.logsumexp(log_rho, dim=0), out=rnm_log_consts)
        ...
        for bi, i in enumerate(range(0, N, self.block_size)):
            ...
            partial_w += calc_W_inv(rowptr, col, rnm_values, X_train, X_test).diagonal(dim1=-1, dim2=-2).sum(dim=0)
            # partial_w += calc_w_contrib(row, col, rnm_values, X_train, X_test)
        
        return partial_w
    
    def partial_step_(self, X, block_idx, sparsity_threshold=None):
        partial_w = self._partial_reestimation_step(X, block_idx, sparsity_threshold=sparsity_threshold)
        with InterruptDelay():
    ...
    def compute_elbo(self, X):
        ...
        expect_log_lambda_p = expect_log_p_lambda(self.nu_0, expect_lambda, expect_log_lambda, self.sigma_0_sq)
    ...
    def log_pred_density(self, X, Y):
        ...
        log_coeff = (lgamma(self.nu + 1 / 2) - lgamma(self.nu)
                     - 1 / 2 * (torch.log(self.w) + math.log(2 * math.pi)))
        # can't use cuda kernels, so we'll use a proportionally smaller batch size
        bsize = math.isqrt(self.block_size ** 2 // D)
        for bi, i in enumerate(range(0, N_train, bsize)):
            X_train = X[i:i + bsize]
            for j in range(0, N_test, bsize):
                Y_test = Y[j:j + bsize]
                log1p_Q = (Y_test[None, :] - X_train[:, None]).pow_(2).mul_(inv_w).log1p_()
                log_probs = log1p_Q.mul_(-(self.nu + 1 / 2)).add_(log_coeff).sum(dim=-1)
                torch.logaddexp(log_densities[j:j + bsize], torch.logsumexp(log_probs, dim=0),
    ...
    def sample(self, X, n):
        ...
        scale = self.w / self.nu
        inds = torch.randint(N, (n,), device=X.device)
        
        # x_hat ~ St(x_n, nu / w, 2nu) is equivalent to
        # x_hat = x_n + y / sqrt(u / (2nu)) where y ~ N(0, (nu / w)^-1) and u ~ Chi^2(2nu)
        # threshold u to avoid very unlikely divide by zero
        y = torch.distributions.Normal(loc=X.new_zeros(D), scale=scale).sample((n,))
        u = torch.clamp_min(torch.distributions.Chi2(df=2 * self.nu).sample((n, D)), eps)
        return X[inds] + y / torch.sqrt(u / (2 * self.nu))
