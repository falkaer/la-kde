...
def graph_to_col(graph, col_offset=0):
    Ng, K = graph.shape
    row = graph.flatten()
    col = torch.arange(col_offset, col_offset + Ng, dtype=graph.dtype, device=graph.device)
...
def graph_to_slices(graph, block_size):
    ...
    for j in range(0, N, block_size):
        col_slices = graph_col_to_slices(graph_to_col(graph[j:j + block_size], j), N, block_size)
