...
from lakde.local_adaptive_kde import LocallyAdaptiveKDE, calc_log_rho_block, expect_log_p_tau, expect_log_q_tau
from lakde.sparse_util import graph_to_slices
from lakde.utils import cov, compute_knn_graph, tensor_as, InterruptDelay
from lakde.linalg_util import mvdigamma, mvlgamma, stabilized_cholesky, chol_logdet, tril_inverse, chol_mm_trace
...
def expect_log_q_lambda(nu, expect_logdet_lambda, logdet_W, D):
    ...
    return torch.sum(expect_log_B_q + (nu - D - 1) * expect_logdet_lambda / 2 - nu * D / 2)
...
def expect_log_q_sigma_shared(nu_shared, logdet_W_shared, expect_logdet_sigma_shared, D):
    ...
    return expect_log_B_q + (nu_shared - D - 1) * expect_logdet_sigma_shared / 2 - nu_shared * D / 2
...
class LocalHierarchicalKDE(LocallyAdaptiveKDE):
    ...
    def init_parameters_(self, X, sparsity_threshold=None):
        ...
        if isinstance(self.k_or_knn_graph, int):
            ...
            self.knn_graph = compute_knn_graph(X, self.k_or_knn_graph, 1024, include_self=True, metric='l2',
                                               dtype=torch.int32)
    ...
    def init_summaries_(self, X, sparsity_threshold=None):
        ...
        for bi, i in enumerate(range(0, N, self.block_size)):
            ...
            expect_tau = self.a[i:i + self.block_size] / self.b[i:i + self.block_size]
            expect_lambda_contrib = W_triu @ W_triu.mT
            expect_lambda_contrib *= (nu * expect_tau)[:, None, None]
            self.expect_lambda_contribs[bi] = torch.sum(expect_lambda_contrib, dtype=torch.double, dim=0)
            ...
            for bj, j in enumerate(range(0, N, self.block_size)):
                ...
                torch.logaddexp(self.rnm_log_consts[j:j + self.block_size], torch.logsumexp(log_rho, dim=0),
        ...
        for bi, i in enumerate(range(0, N, self.block_size)):
            W_inv_contrib, nu = self.calc_posterior_contribs(X, bi)
            W_triu = self.calc_posterior(bi, expect_sigma_bar, W_inv_contrib)
            self.rnm_blocks[bi] = self.partial_expectation_step(X, bi, W_triu, nu, update_normalization=False,
        ...
        self.rnm_log_consts = self.rebalance_responsibilities_(X, -1, self.rnm_log_consts)
    
    def calc_prior_prec(self):
        # W_shared_inv = (self.nu_shared_0 * self.lambda_0
        #                 + self.nu_0 * self.expect_lambda_contribs.sum(dim=0))
        return (self.expect_lambda_contribs.sum(dim=0)
                .mul_(self.nu_0)
    ...
    def partial_step_(self, X, block_idx, sparsity_threshold=None):
        ...
        rnm_indices, rnm_values, rnm_log_consts = self.partial_expectation_step(X, block_idx, W_triu, nu,
                                                                                sparsity_threshold=sparsity_threshold)
        
        logdet_W = chol_logdet(W_triu)
        sigma_lambda_mm_trace = nu * chol_mm_trace(expect_sigma_bar.to(W_triu.dtype), W_triu)
        a = self.a[s:e]
        ...
        expect_lambda_contrib = W_triu @ W_triu.mT
        expect_lambda_contrib *= (nu * expect_tau)[:, None, None]
        expect_lambda_contrib = torch.sum(expect_lambda_contrib, dtype=torch.double, dim=0)
        ...
        with InterruptDelay():
            ...
            W_shared_inv_tril, self.W_shared_inv_jitter = stabilized_cholesky(W_shared_inv, self.W_shared_inv_jitter,
            ...
            self.nu[s:e] = nu
            self.b[s:e] = b
            self.rnm_log_consts = self.rebalance_responsibilities_(X, block_idx, rnm_log_consts)
            
            if self.logger:
                self.logger.add_scalar('z/active_rnm_proportion', self.rnm_active_contribs / N ** 2, self.iter_steps)
                self.logger.add_scalar('z/active_rnm_per_point', self.rnm_active_contribs / N, self.iter_steps)
                self.logger.add_scalar('z/active_rnm_total', self.rnm_active_contribs, self.iter_steps)
    ...
    def compute_elbo(self, X):
        ...
        expect_logdet_lambda = mvdigamma(self.nu / 2, D) + D * math.log(2) + self.logdet_W
        ...
        expect_logdet_sigma_shared = mvdigamma(self.nu_shared / 2, D) + D * math.log(2) + logdet_W_shared
        expect_sigma_shared_lambda_trace = torch.trace(expect_sigma_bar @ expect_lambda_sum)
        expect_sigma_shared_lambda_0_trace = torch.trace(expect_sigma_bar @ self.lambda_0)
        
        expect_log_tau_p = expect_log_p_tau(self.a_0, self.b_0, expect_tau, expect_log_tau)
        expect_log_tau_q = expect_log_q_tau(self.a, self.b, expect_tau, expect_log_tau)
        
        expect_log_lambda_p = expect_log_p_lambda(self.nu_0, expect_logdet_lambda, expect_log_tau,
                                                  expect_logdet_sigma_shared, expect_sigma_shared_lambda_trace, D)
        expect_log_lambda_q = expect_log_q_lambda(self.nu, expect_logdet_lambda, self.logdet_W, D)
        
        expect_log_sigma_shared_p = expect_log_p_sigma_shared(self.nu_shared_0, logdet_sigma_0,
                                                              expect_logdet_sigma_shared,
                                                              expect_sigma_shared_lambda_0_trace, D)
        
        expect_log_sigma_shared_q = expect_log_q_sigma_shared(self.nu_shared, logdet_W_shared,
                                                              expect_logdet_sigma_shared, D)
        ...
        expect_log_sigma_shared_diff = expect_log_sigma_shared_p - expect_log_sigma_shared_q
        
        elbo = expect_log_likelihood_no_rnm + expect_log_lambda_diff + expect_log_z_diff + expect_log_tau_diff + expect_log_sigma_shared_diff
        ...
        if self.logger:
            with InterruptDelay():
                ...
                self.logger.add_scalar('elbo/expect_log_tau_diff', expect_log_tau_diff, self.iter_steps)
                
                self.logger.add_scalar('lambda/expect_log_p_lambda', expect_log_lambda_p, self.iter_steps)
                self.logger.add_scalar('lambda/expect_log_q_lambda', expect_log_lambda_q, self.iter_steps)
                
                self.logger.add_scalar('lambda/expect_log_p_sigma_shared', expect_log_sigma_shared_p, self.iter_steps)
                self.logger.add_scalar('lambda/expect_log_q_sigma_shared', expect_log_sigma_shared_q, self.iter_steps)
                
                self.logger.add_scalar('tau/expect_log_p_tau', expect_log_tau_p, self.iter_steps)
                self.logger.add_scalar('tau/expect_log_q_tau', expect_log_tau_q, self.iter_steps)
                ...
                self.logger.add_scalar('tau/tau_avg', torch.mean(self.a / self.b), self.iter_steps)
                
                self.logger.add_scalar('z/expect_log_p_z', expect_log_z_p, self.iter_steps)
