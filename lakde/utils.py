...
def _compute_knn_graph_generic(X, k, bsize=512, return_dists=False, include_self=False, 
                               metric='l2', dtype=torch.int32):
    N = X.size(0)
    metric = metric.lower()
    ...
    for i in range(0, N, bsize):
        ...
        if return_dists:
            D[i:i + bsize], I[i:i + bsize] = dists.topk(k, dim=1, largest=False, sorted=True)
        else:
...
try:
    ...
    def swig_ptr_from_Int32Tensor(x):
        ...
        return faiss.cast_integer_to_idx_t_ptr(x.storage().data_ptr() + x.storage_offset() * 4)
    ...
    def compute_knn_graph(X, k, bsize=512, return_dists=False, include_self=False, 
                          metric='l2', dtype=torch.int32):
        N = X.size(0)
        metric = metric.lower()
