...
from lakde.linalg_util import chol_logdet, lgamma, logsubexp, mvdigamma, stabilized_cholesky, tril_inverse, xlogx
...
def expect_log_p_tau(a_0, b_0, expect_tau, expect_log_tau):
    return torch.sum(-lgamma(a_0) + a_0 * torch.log(b_0) + (a_0 - 1) * expect_log_tau - b_0 * expect_tau)

def expect_log_q_tau(a, b, expect_tau, expect_log_tau):
    return torch.sum(-lgamma(a) + a * torch.log(b) + (a - 1) * expect_log_tau - b * expect_tau)
...
class LocallyAdaptiveKDE(AbstractKDE, ABC):
    ...
    def partial_expectation_step(self, X, block_idx, W_triu, nu, update_normalization=True, sparsity_threshold=None):
        N, D = X.shape
        s = block_idx * self.block_size
        e = s + self.block_size
        X_train = X[s:e]
        ...
        for bj, j in enumerate(range(0, N, self.block_size)):
            ...
            if update_normalization:
                ...
                torch.logaddexp(logsubexp(old_log_consts[j:j + self.block_size],
    ...
    def log_pred_density(self, X, Y):
        ...
        for bi, i in enumerate(range(0, N_train, self.block_size)):
            ...
            log_coeff = (lgamma((nu + 1) / 2)
                         - lgamma((nu + 1 - D) / 2)
                         + 1 / 2 * chol_logdet(W_triu)
                         - D / 2 * math.log(math.pi)).to(W_triu.dtype)
