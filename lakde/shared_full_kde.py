...
from lakde.linalg_util import chol_mm_trace, lgamma, mvdigamma, chol_logdet, mvlgamma, tril_inverse, xlogx
...
class SharedFullKDE(AbstractKDE):
    ...
    def init_summaries_(self, X, sparsity_threshold=None):
        ...
        for bj, j in enumerate(range(0, N, self.block_size)):
            self.partials_ws[bj] = self.partial_reestimation_step(X, bj, sparsity_threshold=sparsity_threshold)
            if self.verbose:
    ...
    def partial_reestimation_step(self, X, bj, sparsity_threshold=None):
        ...
        for bi, i in enumerate(range(0, N, bsize)):
            ...
            partial_W_inv += calc_W_inv(rowptr, col, rnm_values, X_train, X_test).sum(dim=0)
        
        return partial_W_inv
    
    def partial_step_(self, X, block_idx, sparsity_threshold=None):
        partial_W_inv = self.partial_reestimation_step(X, block_idx, sparsity_threshold=sparsity_threshold)
        with InterruptDelay():
    ...
    def compute_elbo(self, X):
        ...
        sigma_0_lambda_mm_trace = self.nu * chol_mm_trace(self.sigma_0_tril, self.W_triu)
        expect_logdet_lambda = mvdigamma(self.nu / 2, D) + D * math.log(2) + logdet_W
        
        expect_log_lambda_p = expect_log_p_lambda(self.nu_0, expect_logdet_lambda,
                                                  sigma_0_lambda_mm_trace, logdet_sigma_0, D)
        expect_log_lambda_q = expect_log_q_lambda(self.nu, expect_logdet_lambda, logdet_W, D)
    ...
    def log_pred_density(self, X, Y):
        ...
        bsize = self.block_size
        logdet_W = chol_logdet(self.W_triu)
        log_coeff = (lgamma((self.nu + 1) / 2)
                     - lgamma((self.nu + 1 - D) / 2)
                     + 1 / 2 * logdet_W
                     - D / 2 * math.log(math.pi)).to(self.W_triu.dtype)
        log_densities = Y.new_full((N_test,), -math.inf)
        for bi, i in enumerate(range(0, N_train, bsize)):
            X_train = X[i:i + bsize]
            for j in range(0, N_test, bsize):
                Q = chol_quad_form(self.W_triu.expand(X_train.size(0), D, D), X_train, Y[j:j + bsize])
    ...
    def sample(self, X, n):
        ...
        y = torch.distributions.MultivariateNormal(loc=X.new_zeros(D), scale_tril=scale_tril).sample((n,))
        u = torch.clamp_min(torch.distributions.Chi2(df=self.nu + 1 - D).sample((n,)), eps)
